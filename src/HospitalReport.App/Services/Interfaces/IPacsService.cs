...
public interface IPacsService
{
    ...
    // 특정 날짜(보통 오늘) 폴더의 모든 환자 스터디를 '최근 촬영순'으로 반환 (실시간 워크리스트용).
    Task<IReadOnlyList<StudyItem>> GetStudiesForDayAsync(DateTime day, CancellationToken cancellationToken = default);
